...
remote_backup_dir=/home/mbareck/
incremental_dir=~/backup_trush #to preserve a copy of delete file from backup

rsync -vbuzar --backup-dir=$incremental_dir/`date +%F:%T` -e "ssh -p22" $backup_dir $user@$bk_server:$remote_backup_dir --delete
